...
- `dev-vault.tinfoil.sh` as the attested secret broker.

The complete customer deployment is in `broker/`. It pins the canonical
...
named `vault-url` for compatibility; it identifies any compatible attested
secret broker.
...
It bypasses the broker only for local qualification before the CVM image has
...
`--external-config` after the CVM release exists and the attested broker is
...
The broker must authorize the enclave workload repository, not this model
identity or the broker repository. During local pre-release testing,
`tinctl dev-launch` imports its base metadata from `tinfoilsh/confidential-debug`;
after the demo has its own release provenance, configure the broker for
`tinfoilsh/demo-private-model-delivery`.

Broker transport is always HTTPS, including local and air-gapped deployments.
...
Do not tag, upload, or release a locally built CVM image. A successful broker
